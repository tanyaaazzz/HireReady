// controller for enhancing summary
...
//controller for enhancing jon description
    ...
    try {
        ...
        const response = await ai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "You are an exper in resume writing. Your task is to enhance job description of a resume. The job description should be only in 1-2 sentences also highlighting key responsibilities and achievements. Use action verbs and quantifiable results where possible. Make it ATS=friendly. and only return text no option or anything else."
                },
                {
                    role: "user",
                    content: userContent,
...
export const uploadResume = async (req, res) => {
    ...
    education:[
        ...
        const newResume=await Resume.create({userId,title,...parsedData})
         ...
         res.json(200).json({resumeId:newResume._id });
